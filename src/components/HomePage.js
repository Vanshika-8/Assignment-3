...
const HomePage = (props) => {
      ...
      <div key={item.id} className="homepage__cards">
        <div className="homepage">
          ...
          <Link to={`/artwork/${item.id}`}>
            <img className="homepage__images" src={item.path} alt={item.name} />
          </Link>
          <div className="details__homepage">
            <span className="title">{item.name}</span>
            <span style={item.bookmark ? { color: "red" } : { color: "black" }}>
              <FaBookmark
                className="bookmark"
  ...
  return <div className="homePage__list">{getCards()}</div>;
};
